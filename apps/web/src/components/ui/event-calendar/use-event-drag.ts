...
export function useEventDrag({ disabled, onClick, onDrop }: UseEventDragOptions) {
  ...
  const handlePointerMove = useCallback((event: PointerEvent) => {
    ...
    if (draggingRef.current) {
      setActiveTarget(readDropTarget(document.elementFromPoint(event.clientX, event.clientY)));
    }
  }, []);
