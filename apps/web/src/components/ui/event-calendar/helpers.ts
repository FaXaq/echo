...
export function bucketDayEvents(events: CalendarEvent[], maxVisible: number): DayEventBucket {
...
export function getEventBlockPosition(event: CalendarEvent, day: Date): EventBlockPosition {
  const dayStart = dayjs(day).startOf("day");
  const dayEnd = dayjs(day).endOf("day");
  const start = dayjs(event.startDate).isBefore(dayStart) ? dayStart : dayjs(event.startDate);
  const end = dayjs(event.endDate).isAfter(dayEnd) ? dayEnd : dayjs(event.endDate);

  const startMinutes = start.diff(dayStart, "minute");
  const durationMinutes = Math.max(end.diff(start, "minute"), MIN_EVENT_DURATION_MINUTES);
...
export function moveEventToStart(event: CalendarEvent, newStart: Date): CalendarEvent {
...
export function resizeEventEnd(event: CalendarEvent, newEndDate: Date): CalendarEvent {
  const minEnd = dayjs(event.startDate).add(MIN_EVENT_DURATION_MINUTES, "minute");
...
export function currentHourRange(day: Date = new Date()): CalendarEventRange {
  const start = dayjs(day).hour(dayjs().hour()).minute(0).second(0).millisecond(0);
