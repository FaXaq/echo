    ...
    const offPlaybackError = events.on("playbackerror", (payload) => {
      void (async () => {
        ...
        try {
          ...
          // Mirrors recover.loadError's default: without an explicit
          // maxRetries, don't auto-reload. A persistent decode/seek error
          // (e.g. a bad Range response) would otherwise reload at the same
          // currentTime, fail again, and loop indefinitely.
          const maxRetries = options?.maxRetries ?? 0;
          const currentRetryCount = api.getState().asset.retryCount;
          ...
          const decision = options?.recover?.playbackError
            ...
            : currentRetryCount < maxRetries
              ...
              : { action: AssetRecoveryAction.Stop };
